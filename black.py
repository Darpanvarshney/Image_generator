...
def main():
        ...
        def get_text():
            ...
            display_img.configure(text=" ",bg_color="#2B2B2B",image=show_img)
        ...
        canvas.configure(bg="#212121", border=0)

        image = Image.open("components/logo.png")
        ctk_image = ctk.CTkImage(light_image=image, size=(120, 100))
        logo_var = ctk.CTkLabel(canvas, image=ctk_image,width=100,height=100, text="", bg_color="#32383D",corner_radius=40)
        logo_var.place(x=6,y=6)

        image2 = Image.open("components/title.png")
        ctk_image2 = ctk.CTkImage(light_image=image2, size=(370, 110))
        title_name = ctk.CTkLabel(canvas,width=190, height=40,text="",image=ctk_image2,bg_color="#32383D")
        title_name.pack(pady=0)

        bottom = ctk.CTkCanvas(canvas,highlightthickness=0,)
        bottom.pack(side='bottom',padx=10,)
        bottom.configure(border=0,height=100,bg="#212121",width=1300)

        prom = ctk.CTkEntry(bottom, placeholder_text="Enter Your Prompt Here",  width=900 ,font=("Inter",24,), height=80,fg_color="#2F2F2F", border_width=0,corner_radius=50)
        prom.pack(side="bottom",pady=10,)


        button_img = Image.open("components/button.png")
        ...
        button = ctk.CTkButton(bottom, text="",image=ctk_button_image,width=40,height=40,corner_radius=10, 
                                    ...
                                    text_color="white", bg_color="#2F2F2F",hover=None,state="disabled")
        ...
        display_canvas.configure(border=0,height=700,bg="#212121",)

        display_frame = ctk.CTkFrame(display_canvas,height=600,corner_radius=30,bg_color="#212121",fg_color="#2F2F2F")
        display_frame.pack(fill = ctk.BOTH,expand = True , padx =20,pady=20)

        display_img = ctk.CTkLabel(display_frame,text="YOUR GENERATED WILL COME HERE (plz wait only 1 min)",bg_color="#2F2F2F",text_color="#212121",font=("Bold",40))
